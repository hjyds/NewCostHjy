define(["jquery","hammer","jqueryhammer"], function(jquery, hammer, jqueryhammer){
    ...
    function nextday(monthID,monthPreID,monthNextID,dayID,dayPreID,dayNextID,weekID,weekPreID,weekNextID,add_Year,add_Month,add_Day,add_Hours,add_minutes)
    {
        ...
        var theMonth=add_Month,theDay;
        if (add_Day==monthDay) {
            var data=nextmonth(monthID,monthPreID,monthNextID,weekID,weekPreID,weekNextID,add_Year,add_Month,add_Day,add_Hours,add_minutes);
            theMonth=data.theMonth;   //调用month函数
            theDay=1;
            dayID.html(theDay);
        } else{
            theDay=add_Day+1;
        ...
        var dayPre=daySides(add_Year,theMonth,theDay).dayPre,
            dayNext=daySides(add_Year,theMonth,theDay).dayNext;

        dayPreID.html(dayPre);
        dayNextID.html(dayNext);
        weekThree(add_Year,theMonth,theDay,weekID,weekPreID,weekNextID);
        chooseday(add_Year,theMonth,theDay,add_Hours,add_minutes);
        return{
        theMonth:theMonth,
        theDay:theDay
        };
