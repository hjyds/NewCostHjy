...
(function ($) {
    ...
    var ms = $.mobiscroll,
        ...
        defaults = {
            ...
            stepSecond: 1,
            separator: ' ',
			callAfterClose:null,         // 回调函数 停用时刷新当前医嘱列表
			extendParams:{}              // 回调函数 停用时刷新当前医嘱列表 传的参数
        },
                                ...
                                if (regen) {
                                    $('.dw-li', t).each(function () {
										...
										if(str.match(/DD/)){
											...
											if(temp[0] == now.getFullYear() && temp[1] == now.getMonth() && temp[2] == now.getDate() && d == defd.getDate()){
												
											}else{
												nowZH = s.dayNames[w];
											}
											 aaa = str.replace(/DD/, '<span class="dw-day">' + nowZH + '</span>')
										}else{
											aaa = str.replace(/D/, '<span class="dw-day">' + s.dayNamesShort[w] + '</span>')
										}
											$('.dw-i', that).html(aaa);
