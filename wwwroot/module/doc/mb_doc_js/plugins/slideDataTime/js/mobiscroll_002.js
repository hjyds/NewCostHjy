...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        function position(check) {
            ...
            $('.dw-persp', dw).height(0).height(t + mh > $(document).height() ? t + mh : $(document).height()).css("display","block");  //先隐藏
        ...
        */
        that.show = function (prevAnim) {
            ...
            if (s.display != 'inline') {
				...
				// 如果是医嘱停止
				if(elm.attr("id")=="timeControlWrapInput"){
					debugger;
					//showHide("医嘱停止按钮触发");
					yuanLai();
					$("#timeControlWrapKJ").append($(".android-ics"));
					$("#timeControlWrap").show();
					$("#timeControlWrapF").show();
				// 如果是新开/编辑
				}else if(elm.attr("id")=="appDateTime"){
					//showHide("医嘱编辑/新开按钮触发");
					yuanLai();
					$("#startTimeLi").css("background","#dffbfb");
					$("#appDateTime").css("background","#dffbfb");
					$(".yzxdKongJian").hide();
					$("#startTime").append($(".android-ics"));
					$("#startTime").show();
				}
            } else if (elm.is('div')) {
                elm.html(dw);
            ...
            if (s.display != 'inline') {
					...
					if(!$("#timeControlWrap").is(":hidden")){
						...
						$.ajax({
                            ...
                            data: JSON.stringify({
                                "IN": {
                                    "YZID": yzID,
                                    "TZSJ": $("#timeControlWrapInput").val(),
                                    "TZYS": tzYS
                                }
                            }),
							success: function(responseTxt){
								if(responseTxt.result.ERROR){
									showHide(responseTxt.result.ERROR.MSG);
								}else{
									showHide("停用成功");
									//docActListLoad("yes",0); // 刷新当前病人的医嘱列表
									if(settings.callAfterClose && settings.extendParams){
										settings.callAfterClose(settings.extendParams.dellTr, settings.extendParams.startPage);
									}
									// 关闭编辑状态
				...
				// 点击背景层关闭停用控件
				$("#timeControlWrapF").on("click",function(){
					that.cancel();
					$("#timeControlWrap").hide();
					$("#timeControlWrapF").hide();
				});
            ...
            if (s.display == 'inline') {
                ...
                if (input && s.showOnFocus) {
					...
					elm.bind('click', function (event) { 
						...
						//showHide("触发click事件");
						if(elm.attr("id")=="timeControlWrapInput"){
							if(!$("#yzButtonTZhi").hasClass("noEdit")){
