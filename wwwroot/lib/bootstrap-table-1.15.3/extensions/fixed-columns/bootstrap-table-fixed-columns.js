(function (global, factory) {
	...
	*/
	...
	//20220827byZT：固定列滚动事件被翻倍触发，removeEventListener无效，用时间判断规避
	var mouseWheelTimeStamp = 0;
	...
	function (_$$BootstrapTable) {
	  ...
	  _createClass(_class, [{
	    ...
	    value: function trigger() {
	      ...
	      if (!this.fixedColumnsSupported()) {
			...
			if (args[0] === 'post-header') {
			  //20220827byZT：开放fitHeader命令，可指定不处理表体
			  this.initFixedColumnsHeader(args[1]);
		...
		//20220827byZT：开放fitHeader命令，可指定不处理表体
	    value: function initFixedColumnsHeader(dontSetBody) {
	      var _this2 = this;
	      ...
	      var initFixedHeader = function initFixedHeader($fixedColumns, isRight, dontSetBody) {
			$fixedColumns.find('.fixed-table-header').remove();
			if (dontSetBody) {
				$fixedColumns.prepend(_this2.$tableHeader.clone(true));
            } else {
	      ...
	      if (this.needFixedColumns && this.options.fixedNumber) {
	        this.$fixedHeader = initFixedHeader(this.$fixedColumns, undefined, dontSetBody);
	      ...
	      if (this.needFixedColumns && this.options.fixedRightNumber) {
	        this.$fixedHeaderRight = initFixedHeader(this.$fixedColumnsRight, true, dontSetBody);
	        this.$fixedHeaderRight.scrollLeft(this.$fixedHeaderRight.find('table').width());
	      } else if (this.$fixedColumnsRight) {
	        this.$fixedColumnsRight.html('').css('width', '');
	      }

		  if (!dontSetBody || this.$fixedColumns.find('.fixed-table-body').length==0) {
			this.initFixedColumnsBody();
          }
	      this.initFixedColumnsEvents();
	    }
	    ...
	    value: function initFixedColumnsBody() {
	      ...
	      var initFixedBody = function initFixedBody($fixedColumns, $fixedHeader) {
			...
			//20220827byZT：开放fitHeader命令，即使同步处理表体，也同步滚动
			$fixedBody.scrollTop(_this3.$tableBody.scrollTop());
	    ...
	    value: function initFixedColumnsEvents() {
	      ...
	      if (this.needFixedColumns && this.options.fixedNumber) {
			...
			this.$fixedBody[0].addEventListener(mousewheel, function (e) {
			  if (e.timeStamp == mouseWheelTimeStamp) return;
